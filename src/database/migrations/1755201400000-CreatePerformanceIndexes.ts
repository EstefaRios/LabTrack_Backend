...
export class CreatePerformanceIndexes1755201400000
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_notification_id_usuario_leida_fecha"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_notification_type_fecha"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_notification_leida_fecha"`,
    );

    // Eliminar índices de audit_log
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_audit_log_created_at_id_usuario"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_audit_log_table_name_created_at"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_audit_log_accion_created_at"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_audit_log_id_usuario_table_name"`,
    );
  }
}
